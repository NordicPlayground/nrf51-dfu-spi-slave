...
static uint32_t                   * m_storage_handle_sd;
static uint32_t                   * m_storage_handle_app;       
static uint32_t                   * mp_storage_handle_active; 
...
uint32_t dfu_start_pkt_handle(dfu_update_packet_t * p_packet)
{
    ...
    if (m_start_packet.bl_image_size > (uint32_t)DFU_BL_IMAGE_MAX_SIZE)
    {
    ...
    switch (m_dfu_state)
    {
        case DFU_STATE_RX_DATA_PKT:
            ...
            else
            {
                ...
                if (err_code == NRF_SUCCESS)
                {
                    err_code = dfu_init_postvalidate((uint8_t *)mp_storage_handle_active,
                                                     m_image_size);
                    if (err_code != NRF_SUCCESS)
                    {
                        return err_code;
